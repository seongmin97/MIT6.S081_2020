...
// map kernel page table in process 
...
// Free kernel page tables in processes
...
void _helper_vmprint(pagetable_t pagetable, int depth){
  char * prefix_s;
  switch (depth){
  ...
  for(int i=0; i<512; i++){
    ...
    printf("%s%d: pte %p pa %p\n", prefix_s, i, pte, child);
    if((pte & (PTE_R|PTE_W|PTE_X)) == 0) _helper_vmprint((pagetable_t)child, depth+1);
  }
}
